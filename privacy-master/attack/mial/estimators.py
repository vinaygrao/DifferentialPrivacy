...
class CustomModelBundle(sklearn.base.BaseEstimator):
    """
    ...
    def _fit(self, X, y, verbose=False, pseudo=True, fit_kwargs=None):
        ...
        self.shadow_train_indices_= train_indices
        self.shadow_test_indices_=test_indices
    ...
    def _transform(self,train_data, test_data,  shadow_indices=None, verbose=False):
        ...
        shadow_model = self
        train_indices = self.shadow_train_indices_
        test_indices = self.shadow_test_indices_
...
class ShadowModelBundle2(sklearn.base.BaseEstimator):
    """
    A bundle of shadow models.
    ...
    def _fit(self, X, y, verbose=False, pseudo=True, fit_kwargs=None):
        ...
        for i in self._get_model_iterator(verbose=verbose):
            # Pick indices for this shadow model.
            shadow_indices = self._prng.choice(
                indices, 2 * self.shadow_dataset_size, replace=True
            )
            train_indices = shadow_indices[: self.shadow_dataset_size]
            test_indices = shadow_indices[self.shadow_dataset_size :]
            X_train, y_train = X[train_indices], y[train_indices]
